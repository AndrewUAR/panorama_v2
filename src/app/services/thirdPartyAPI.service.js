...
export const getPlaces = async (place) => {
  ...
  const response = await axios.post(`${API_ENDPOINT}/api/v1/data/getPlaces`, place);
  store.dispatch(asyncActionFinish());
  return response;
}

export const getMyPlace = async (coordinates) => {
  const response = await axios.post(`${API_ENDPOINT}/api/v1/data/getMyPlace`, coordinates);
  store.dispatch(asyncActionFinish());
  return response;
}